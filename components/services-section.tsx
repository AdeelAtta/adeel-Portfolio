      ...
      <div className="container mx-auto px-4">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12 md:mb-16">
            <h2 className="text-4xl md:text-[52px] md:leading-15 font-bold mb-4">
              Technical <span className="bg-[#FF4A60] text-white px-3 py-1 inline-block">expertise</span>
            </h2>
            <p className="text-[#393939] text-base md:text-lg font-medium leading-relaxed md:leading-7.5 max-w-2xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {primarySkills.map((skill, index) => (
              <div
                key={index}
                className="bg-white border-[3px] border-black rounded-4xl overflow-hidden hover:-translate-y-1 hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transition-all duration-300 min-h-120 flex flex-col group"
              >
                <div className="mb-6 -mx-0.75 -mt-0.75 overflow-hidden rounded-t-[29px]">
                  <Image
                    src={skill.image || "/placeholder.svg"}
                    alt={`${skill.title} - Technical expertise illustration`}
                    width={382}
                    height={328}
                ...
                <div className="px-8 pb-8 flex-1 flex flex-col">
                  <h3 className="text-[28px] leading-10 font-bold mb-3 text-[#0B0B0B]">{skill.title}</h3>
                  <p className="text-[18px] leading-7.5 font-medium text-[#393939]">{skill.description}</p>
                </div>
              </div>
            ))}

            <div className="bg-[#FFC224] border-[3px] border-black rounded-4xl p-8 md:p-12 flex flex-col items-center justify-center text-center hover:-translate-y-1 transition-transform min-h-120 relative shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
              <div className="mb-6">
              ...
              <h3 className="text-[28px] leading-10 font-bold mb-4 text-[#0B0B0B]">{"Let's collaborate"}</h3>
              <p className="text-[18px] leading-7.5 font-medium text-[#393939] mb-8">
                Have a project in mind? I'd love to hear about it and explore how we can work together.
              </p>
              <a href="mailto:adeelatta2000@gmail.com" className="w-full max-w-85">
                <Button className="bg-black text-white hover:bg-black/90 rounded-2xl px-12 py-6 font-medium text-[18px] w-full h-16">
